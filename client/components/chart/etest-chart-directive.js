    ...
    link: function (scope, element, attrs) {
      scope.$watch('type', function (newType, oldType) {
        ...
        if (newType == 'pie') {
          ...
          $(element).highcharts(ChartService.getChart(scope.title, scope.type, categories ? data : scope.data, categories ? categories : undefined, scope.theme));
        }
      ...
      scope.$watch('data', function (data, old) {
        ...
        if (scope.type == 'pie') {
          ...
          $(element).css({width: '98%', height: 'auto', margin: '0 auto'});
          $(element).highcharts(ChartService.getChart(scope.title, scope.type, categories ? rankData : data, categories, scope.theme));
        }
